    ...
    */
        ...
        MsgId topicRootId)
    {
        ...
        const auto showDocument = [&]
        {
            ...
            {
                auto url = GetBaseUrl();
                QString mpvFilePath = QCoreApplication::applicationDirPath() + "/mpv.txt";
                QFile file(mpvFilePath);

                if (url != "")
                {
                    const auto channel = item->history()->peer->asChannel();
                    auto peer = (channel && channel->hasUsername()
                                     ? channel->username()
                                     : QString::number(msgId.peer.value));
                    auto mediaUrl = url + peer + "/" + QString::number(msgId.msg.bare);
                    if (file.exists())
                    {
                        bool success = QProcess::startDetached("mpv.exe", QStringList() << mediaUrl);
                    }
                    else
                    {
